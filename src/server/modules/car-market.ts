...
export default class CarMarket {
	...
	constructor(creationAttrs: CarMarketCreation) {
		...
		const upperLeft = new mp.Vector3(position.x - dimensions.width / 2, position.y + dimensions.depth / 2, position.z);
		const upperRight = new mp.Vector3(position.x + dimensions.width / 2, position.y + dimensions.depth / 2, position.z);

		// Placing the sell points on the perimeter
		let sellPointNumber = 1;

		// from upper left to upper right (with offsets)
		for (let xPos = upperLeft.x + BOUND_OFFSET; xPos < upperLeft.x + dimensions.width - BOUND_OFFSET; xPos += STEP) {
			const pos = new mp.Vector3(xPos, upperLeft.y - BOUND_OFFSET, upperLeft.z);

			this._sellPoints.push(
				new SellPoint({
					position: pos,
