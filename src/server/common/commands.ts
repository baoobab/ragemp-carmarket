...
mp.events.addCommand('owncars', (player: PlayerMp) => {
	player.outputChatBox(
		`own cars count: ${player.ownVehicles.length}\nmodels: ${player.ownVehicles.map((veh) => {
			return veh.model;
		})}`
	);
});
...
mp.events.addCommand('sellcar', (player: PlayerMp, fullText) => {
	...
	ownVehicle.destroy();
	player.ownVehicles.splice(
		player.ownVehicles.findIndex((veh) => veh.id === destroyVehicleId),
		1
	);
